 ...
 */
@Component
public class EBankConfParser4esb implements IEBankConfParser {
	private Logger logger = YTLog.getLogger(EBankConfParser4esb.class);

	@Autowired
	private IEBankConfCaches ebankConfCaches;
	private SAXReader reader;
	private final String rootpath = "/WEB-INF/conf/esb/";

	public EBankConfParser4esb() {
		...
		if (!ebankConfCaches.hasTransConfById(id)) {
			...
			if (doc != null && doc.hasContent()) {
				...
				logger.debug("load trancation defined file desc: "
						+ root.attributeValue(ESBConst.AT_NAME));
	...
	public MBTransConfBean parserObject(Element el) {
		...
		conf.setName(el.attributeValue(ESBConst.AT_NAME));

		List<Element> list;
		if (el.hasContent()) {
			list = el.elements();
			for (Element e : list) {
				if (e.getName().equals(ESBConst.XT_SEND)){
					parserSnd(e, conf);
				} else if (e.getName().equals(ESBConst.XT_RCV)) {
					parserRce(e, conf);
				} else if (e.getName().equals(ESBConst.XT_SEND_HEAD)) {
					parserSndHeader(e, conf);
				} else if (e.getName().equals(ESBConst.XT_RCV_HEAD)) {
	...
	public void parserSnd(Element el, MBTransConfBean conf) {
		...
		for (Element e : list) {
			...
			if(ESBConst.XT_head.equals(e.getName())){
			...
			}else if(ESBConst.XT_body.equals(e.getName())){
			...
			} else if(ESBConst.XT_ITEM.equals(e.getName())){
				item = new MBTransItem();
	...
	public void parserSndHeader(Element el, MBTransConfBean conf) {
		List<Element> list = el.elements();
		for (Element e : list) {
			if (e.getName().equals(ESBConst.XT_ITEM)) {
				// 仅支持非列表的字段解析
	...
	private boolean parseCommonItem(Element e, MBTransItem item) {
		String name = e.attributeValue(ESBConst.AT_NAME);
		String targetName = e.attributeValue(ESBConst.AT_TARGET_NAME);
		String client = e.attributeValue(ESBConst.AT_CLIENT);
		String desc = e.attributeValue(ESBConst.AT_DESC);
		String type = e.attributeValue(ESBConst.AT_TYPE);
		String dolt = e.attributeValue(ESBConst.AT_DOLT);
		String length = e.attributeValue(ESBConst.AT_LEN);
		String required = e.attributeValue(ESBConst.AT_REQUIRED);
		String mapkey = e.attributeValue(ESBConst.AT_MAPKEY);
		String descName = e.attributeValue(ESBConst.AT_MAPKEY_DESCNAME);
		String xpath = e.attributeValue(ESBConst.AT_XPATH);
		String defaultValue = e.attributeValue(ESBConst.AT_DEFVAL);
		String plugin = e.attributeValue(ESBConst.AT_PLUS);
		String delimiter = e.attributeValue(ESBConst.AT_DELIMITER_ROW);
		String itemDelimiter = e.attributeValue(ESBConst.AT_DELIMITER_FIELD);
		String sizeField = e.attributeValue(ESBConst.AT_SIZE_FIELD);
		item.setName(name);
		item.setType(type);
	...
	private boolean isListItem(String type) {
		return ESBConst.FILED_TYPE_E.equalsIgnoreCase(type) || ESBConst.FILED_TYPE_D.equalsIgnoreCase(type) || ESBConst.FILED_TYPE_S.equalsIgnoreCase(type);
	}
