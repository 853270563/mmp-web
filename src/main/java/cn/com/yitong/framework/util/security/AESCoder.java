...
public class AESCoder {
	...
	public static void main(String[] args) throws Exception {
		...
		Key k = AESCoder.toKey("1234567890123456".getBytes("UTF-8"));

		String data = "0123456789";
		System.out.println("加密前数据: string:" + data);
		System.out.println("加密前数据: 字节:" + data.getBytes("UTF-8").length);
		System.out.println("加密前数据: byte[]:" + showByteArray(data.getBytes()));
		System.out.println();
		
		byte[] encryptData = AESCoder.encrypt(data.getBytes("UTF-8"), k);
		System.out.println("加密后数据: byte[]:" + showByteArray(encryptData));
		System.out.println("加密后数据: 字节:" + encryptData.length);
		System.out.println("加密后数据: hexStr:"
				+ Converts.base64ToString(encryptData));
		System.out.println();
