...
@Component
public class ResponseParser4credit implements IResponseParser {
	...
	private boolean parseService(String transCode, Element response,
		...
		if (null != conf) {
			...
			for (MBTransItem item : rcv) {
				...
				if (CreditConst.FILED_TYPE_E.equals(itemType)) {
					// 列表内容
				...
				} else if (CreditConst.FILED_TYPE_S.equals(itemType)) {
					// 有子元素的节点
					List listNodes = response.selectNodes(xpath);
					if (null == listNodes || listNodes.size()<=0) {
						continue;
				...
				} else if(CreditConst.FILED_TYPE_C.equals(itemType)){
					// 普通内容
					node = response.selectSingleNode(xpath);
					if (null != node) {
						busiContext.saveNode(rctx,clientName, node.getText().trim());
					} else{ 
						busiContext.saveNode(rctx, clientName,CreditConst.EMPTY);
					}
				}else if(CreditConst.FILED_TYPE_T.equals(itemType)){
					node = response.selectSingleNode(xpath);
					if (null != node) {
						busiContext.saveNode(rctx,clientName,node.valueOf("@total"));
					} else{ 
						busiContext.saveNode(rctx, clientName,CreditConst.EMPTY);
					}
				...
				for (MBTransItem mapItem : childMap) {
					...
					if (CreditConst.FILED_TYPE_E.equals(type)) {
						...
						if (null != tag) {
							...
							} else if (mapItem.isRequired()) {
								busiContext.saveNode(parentElement, clientName, CreditConst.EMPTY);
							}
						}
					}
				}
	...
	private boolean parserListData(String transCode, MBTransItem item,
		...
		if (nodes == null || nodes.isEmpty()) {
			...
			List<MBTransItem> childMap = item.getChildren();
			Element listElem = parentElement.addElement(item.getClient());
			listElem.addAttribute(CreditConst.AT_TYPE, CreditConst.TYPE_LIST);
			// 遍历列表实体
			for (Node node : nodes) {
				Element mapElem = listElem.addElement(AppConstants.XML_MAP);
				mapElem.addAttribute(CreditConst.AT_TYPE, CreditConst.TYPE_MAP);
				// 遍历实体结构
				for (MBTransItem mapItem : childMap) {
					...
					if (CreditConst.FILED_TYPE_E.equals(type)) {
						...
						if (null != tag) {
							...
							if (StringUtil.isNotEmpty(text)) {
								...
								busiContext.saveNode(mapElem, clientName, CreditConst.EMPTY);
							}
						}
					}
				}
