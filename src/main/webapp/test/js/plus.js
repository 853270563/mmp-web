		...
		for (var i = 0, j = grps.length; i < j; i++) {
			...
			for (var i2 = 0; i2 < size; i2++) {
				...
				for (var i3 = 0; i3 < keySize; i3++) {
					var key = grpItems[i3];
					 var mapValue= grpItemDatas[key][i2]
					if(mapValue){
						map[key] =mapValue ;
					}
				}
				if(Object.keys(map).length!=0){
					if(grpItems.isMap){
						sendObj[grpName] = map;//实体结构
					}else if(Object.keys(map).length==1&&keySize==1&&key=='String'){
						datas.push(map[Object.keys(map)[0]]);
					}
					else{
