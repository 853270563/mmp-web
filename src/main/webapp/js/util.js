...
function getFormJson(panel,sendObj){
	...
	panel.find("[data-type]").each(function(){
		...
		if("radio"==dataType){
		...
		}else if(dataType=="oldradios"){//问卷调查
			name=jqObj.attr("data-name");
			var check=jqObj.attr("checked");
			if(check=="checked"){
				value=jqObj.attr("data-value");
			}
		}else if(dataType=="oldcheckbox"){// input type=checkbox问卷调查
			var str="";
			name=jqObj.attr("name");
			panel.find("input[name='"+name+"']:checked").each(function(index){
				 if(panel.find("input[name='"+name+"']:checked").length==index+1){
					 str += jqObj.val();
				 }else{
					 str += jqObj.val()+"T";
				 }
			...
			name=jqObj.attr("data-name");
			value=jqObj.html();
		}else if("questradio"==dataType){
			var str="";
			name=jqObj.find("input").eq("0").attr("data-name");
			panel.find("input[name='"+name+"']:checked").each(function(index){
				 if(panel.find("input[name='"+name+"']:checked").length==index+1){
					 str += $(this).val();
				 }else{
					...
					try{
						if (rpdata && rpdata.STATUS) { 
							if (rpdata.STATUS == "005") {
								...
								//run = false; // create by zhanglong 2015-11-13 测试界面提示005 超时 同时也显示返回结果
		...
		if (_ajax.timeout > 0) {
			_ajax._timeoutHandle = setTimeout(function() {
				...
				if (xhr && _ajax.timeoutflg) {
					...
					//showMsgPanel("系统繁忙，资金类交易请及时核对賬戶信息!\n"+url);
					alert("交易超时！");
				}
			}, _ajax.timeout);
		}
...
/**
 * 更改大豐銀行的 10 位 12位 14位 16位 的帐号到17位 存储的时候用
 * @param accStr
 * @returns
 */
