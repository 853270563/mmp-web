 ...
 */
@Component
public class EBankConfParser4mb implements IEBankConfParser {
	private Logger logger = YTLog.getLogger(this.getClass());

	@Autowired
	private IEBankConfCaches ebankConfCaches;
	private SAXReader reader;
	private final String rootpath = "/WEB-INF/conf/mb/";

	public EBankConfParser4mb() {
		//logger.info("加载通讯报文定义数据文件路径:" + rootpath);
...
//		if (!ebankConfCaches.hasTransConfById(id)) {
		if (true) {
			...
			if (doc != null && doc.hasContent()) {
				...
				logger.info("load trancation defined file desc: "
	...
	public MBTransConfBean parserObject(Element el) {
		...
		transConf.setPropery(EBankConst.AT_REQ_SERVICE,
				el.attributeValue(EBankConst.AT_REQ_SERVICE));

		transConf.setPropery(EBankConst.AT_RSP_SERVICE,
				el.attributeValue(EBankConst.AT_RSP_SERVICE));
	...
	public void parserSnd(Element el, MBTransConfBean transConf) {
		...
		for (Element e : list) {
			...
			logger.info("send item :" + item.toString());
	...
	public void parserRce(Element el, MBTransConfBean transConf) {
		...
		for (Element e : list) {
			MBTransItem item = new MBTransItem();
			parseCommonItem(e, item);
			logger.info("rcv item :" + item.toString());
			if (isListItem(e)) {
				// 如果类型是list 则进行 子节点解析
				...
				for (Element mapel : children) {
					...
					logger.info("subItem item :" + subItem.toString());
	...
	public void parserRceHeader(Element el, MBTransConfBean transConf) {
		...
		for (Element e : list) {
			MBTransItem item = new MBTransItem();
			parseCommonItem(e, item);
			logger.info("rcv Header item :" + item.toString());
			if (isListItem(e)) {
				// 如果类型是list 则进行 子节点解析
				List<Element> children = e.elements();
				List<MBTransItem> subList = new ArrayList<MBTransItem>();
				for (Element mapel : children) {
					MBTransItem subItem = new MBTransItem();
					parseCommonItem(mapel, subItem);
					logger.info("subItem Header item :" + subItem.toString());
					parseListChildItem(subItem, mapel);
					subList.add(subItem);
				}
				item.setChildren(subList);
