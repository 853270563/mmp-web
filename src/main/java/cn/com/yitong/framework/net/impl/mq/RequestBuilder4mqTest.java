...
@Component
public class RequestBuilder4mqTest implements IRequstBuilder {
	...
	public boolean buildSendMessage(IBusinessContext busiContext,
		...
		busiContext.setRequestEntry("<?xml version=\"1.0\" encoding=\"UTF-8\"?>"+reqCtx.element("EaiMsg").asXML());
	...
	private boolean buildDocument(IBusinessContext ctx, MBTransConfBean conf,
		...
		try {
			...
			// 设置交易报文体
			return buildBodyContent(elemEaiFrmData, ctx, conf, transCode);
		} catch (Exception e) {
