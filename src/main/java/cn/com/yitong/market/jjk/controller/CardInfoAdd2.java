...
@Controller
public class CardInfoAdd2 extends BaseControl {
	...
	public Map list(String trans_code, HttpServletRequest request) {
		...
		try {
			...
			isOK=service.insert(statementName, params);
		} catch (Exception e) {
			// 输出错误的关键信息
			logger.error(ctx.getTransLogBean(transCode), e);
			bizLogger.warn("用户开卡申请失败，失败原因:"+e.getMessage(),"321102");
			isOK = false;
		}
		transAfter(ctx, transCode, rst, isOK);
		if(isOK == true){
