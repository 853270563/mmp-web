 ...
 * @author yaoym
 */
public class MqClient4dsb implements IClientFactory {

	private Logger logger = YTLog.getLogger(MqClient4dsb.class);
	...
	public void init() {
		MQEnvironment.hostname = "172.31.252.141";
		MQEnvironment.port = 1414;
		MQEnvironment.channel = "CHANL_DSB";// 前端渠道
		...
		// inQName = "Q_DSM_IN"; // 写入队列
		inQName = "Q_DSM_IN"; // 写入队列
		outQName = "Q_IBANK_IN"; // 响应队列Q_DSM_IN|Q_IBANK_OUT
	...
	public boolean execute(IBusinessContext ctx, String transCode) {
		// 测试请求报文生成
		Element rqctx = ctx.getRequestContext(transCode);
	...
	public static void main(String[] args) throws InterruptedException {
		MqClient4dsb util = new MqClient4dsb();
		util.init();
