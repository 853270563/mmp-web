...
public class ComServerSocketcredit{
	...
	public boolean init(String serverIP, int serverPort) {
		try {
			...
			throw new RuntimeException("无法找到该服务器:CRM服务器");
		} catch (IOException ex) {
			logger.warn("无法和服务器建立socket连接：" + serverIP + ":" + serverPort);
			throw new RuntimeException("无法和服务器建立socket连接：CRM服务器");
		}
	}
	...
	public boolean sendMessage(String message) {
		try {
			String lengthStr=""+message.getBytes(CrmConst.UTF_8).length;
			String headNumSre=StringUtil.lpadString(lengthStr, CrmConst.LENGTH, "0");
			message=headNumSre+message;
			int len = message.getBytes(CrmConst.UTF_8).length;
			logger.info("send string message:\n " + message + "$");
			logger.info("报文总长度: " + len);
			outputStream.write(message.getBytes(CrmConst.UTF_8), 0, len);
			outputStream.flush();
			return true;
		} catch (IOException ex) {
	...
	public String receiveMessage() throws IOException {
		byte[] rspLength = new byte[CrmConst.LENGTH];
		...
		return new String(rspContent, CrmConst.UTF_8); // , "Cp937");
