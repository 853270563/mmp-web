...
@Component
public class NetConnect4As implements IClientFactory{
...
//	private AutoTestTool att;
	...
	public boolean execute(IBusinessContext ctx, String transCode) {
		...
		String isurl = (String) ctx.getParamMap().get("isurl");
		String path;
		if(null != isurl && isurl.equals("1")){
			path = serverURL2;
		}else{
			path = serverURL;
		}
...
//		PrintWriter out = null;
		...
		String req = null;
		String rsp = null;
		String url = StringUtil.message(path, excode);
		try {
			cnt.getAndIncrement();
			running = true;
...
//			out.print(req);
			...
			throw new AresRuntimeException("net.connect.error");
		} catch (IOException e) {
			logger.error(serverURL, e);
			throw new OtherRuntimeException(AppConstants.STATUS_FAIL, "数据采集平台服务服务异常");
		} finally {
			if (running) {
				running = false;// 单线程锁
			}
	 ...
	 * @return
	 */
//	private String filterContent(String request) {
//		// 交易流水, 交易时间
////		if (!att.canSaveRspData()) {
////			return request;
////		}
//		request = request.replaceAll("\\\"H_CUST_NUM\":\\\".*?\\\"", "");
//		request = request.replaceAll("\\\"H_TRANS_DATE\":\\\".*?\\\"", "");
//		return request;
//	}
