...
public class DeviceInfoService {
	...
	private static final Long MAX_USER_BOUND_DEVICE_NUM = 4L;
	...
	public Boolean boundDevice(Map params, Map rst) {
		...
		Long max = null;
		String maxNum = Properties.getString("maxUserBoundDevice");
		if(StringUtil.isNotEmpty(maxNum)) {
			max = Long.valueOf(maxNum);
		}else {
			max = MAX_USER_BOUND_DEVICE_NUM;
		}
		if(cnt >= max) {
