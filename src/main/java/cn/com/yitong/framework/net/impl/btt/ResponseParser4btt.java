 ...
 */
@Component
public class ResponseParser4btt implements IResponseParser {
	...
	public boolean parserResponseData(IBusinessContext busiContext,
		...
		if (logger.isDebugEnabled()) {
			logger.debug(transCode + " receive data:\n"
			...
			for (MBTransItem item : rcv) {
				...
				if (EBankConst.FILED_TYPE_E.equals(item.getType())) {
					...
					if (null != tag) {
						...
						busiContext.saveNode(rctx, item.getTargetName(), text);
						parserItemDesc(busiContext, item, text, rctx);
			...
			for (Node entry : entrys) {
				...
				for (MBTransItem mapItem : childMap) {
					...
					if (EBankConst.FILED_TYPE_E.equals(type)) {
						List<Node> listNode = entry.selectNodes(mapItem.getXmlPath());
	...
	public boolean parserResponseStatus(Element rootel, IBusinessContext ctx,
		...
		Node rstCode = rootel.selectSingleNode("respData/code");// 状态码
		Node rstMsg = rootel.selectSingleNode("respData/message");// 返回信息
		...
		if (rspCode.equals(AppConstants.AAAAAA)) {
			ctx.setErrorInfo(AppConstants.STATUS_OK, rspMsg, transCode);
			return true;
		}
		ctx.setErrorInfo(rspCode, rspCode + ":" + rspMsg, transCode);
		return false;
	}
